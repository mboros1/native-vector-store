...
Included tools
- `vsx-scrape`: Scrape Open VSX (VS Code extension marketplace mirror) and emit a JSON dump of extensions suitable for client-side indexing.

Quick start
- Everything: `cargo run -p tiny-search-engine --bin vsx-scrape --release -- --all --query "*" --page-size 100 --concurrency 6 --pretty > vsx.json`
- Paged sample: `cargo run -p tiny-search-engine --bin vsx-scrape --release -- --pages 3 --page-size 100 --concurrency 4 --pretty > vsx.json`
