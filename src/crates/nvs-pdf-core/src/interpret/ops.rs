    ...
    while let Some(tok) = toks.next()? {
        match tok {
            Tok::Op(op) => match op.as_str() {
                "BI" => {
                    handle_inline_image(&mut toks, &mut last_nums);
                }
                "BT" => {
                    begin_text(&mut ts, &mut last_nums);
                }
                "T*" => {
                    new_line(&mut out, &mut last_nums);
                }
                "ET" => {
                    end_text_block(&mut out, &mut last_nums);
                }
                "Tm" => {
                    apply_tm(&mut ts, &last_nums, &mut out);
                    last_nums.clear();
                }
                "Td" | "TD" => {
                    apply_td(&mut ts, &last_nums, &mut out);
                    last_nums.clear();
                }
                "TL" => {
                ...
                "Do" => {
                    handle_do_form(doc, xobjects, fonts, &mut pending_name, depth, &mut out)?;
                    last_nums.clear();
                }
                "Tf" => {
                    set_font(&mut current_font, &mut ts, &last_nums, &mut pending_name);
                    last_nums.clear();
                }
                _ => {
                    last_nums.clear();
...
fn handle_inline_image(toks: &mut Tokenizer, last_nums: &mut Vec<f64>) {
    loop {
        if let Some(next) = toks.next().ok().flatten() {
...
fn begin_text(ts: &mut TextState, last_nums: &mut Vec<f64>) {
    ts.tm_e = 0.0;
    ts.tm_f = 0.0;
    ts.prev_y = None;
    last_nums.clear();
}
fn new_line(out: &mut String, last_nums: &mut Vec<f64>) {
    out.push('\n');
    last_nums.clear();
}
fn end_text_block(out: &mut String, last_nums: &mut Vec<f64>) {
    if !out.ends_with('\n') {
        out.push('\n');
    }
    last_nums.clear();
...
fn set_font(
    current_font: &mut Option<String>,
    ts: &mut TextState,
    last_nums: &Vec<f64>,
        ...
        if let Some(xv) = xobjects.get(&nm) {
            ...
            if let PdfValue::Stream { dict, data } = rv {
                if as_name(dict.get("Subtype").unwrap_or(&PdfValue::Null)) == Some("Form") {
                    if let Some((dec, sub_xobjs, sub_fonts)) =
                        resolve_form_and_merge_resources(doc, &dict, data, xobjects, fonts)
                    {
