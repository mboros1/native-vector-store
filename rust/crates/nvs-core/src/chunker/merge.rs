...
pub fn merge_small_chunks(chunks: Vec<TmpChunk>, min_tokens: usize, max_tokens: usize) -> Vec<TmpChunk> {
    ...
    while i < chunks.len() {
        ...
        while cur.tokens < min_tokens && i + 1 < chunks.len() {
            ...
            let allow = if combined_tokens <= max_tokens { true } else { false };
            if !allow { break; }
...
pub fn final_merge(chunks: Vec<TmpChunk>, min_tokens: usize, max_tokens: usize) -> Vec<super::Chunk> {
    ...
    out.into_iter().map(|tc| super::Chunk {
        text: tc.text,
        token_count: tc.tokens,
        start_page: tc.start_page,
        end_page: tc.end_page,
        has_major_heading: tc.has_major_heading,
        min_heading_level: if tc.min_heading_level == i32::MAX { 0 } else { tc.min_heading_level },
    }).collect()
}
