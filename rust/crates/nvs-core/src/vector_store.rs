...
impl VectorStore {
    ...
    pub fn size(&self) -> usize { self.bundle.manifest.num_docs as usize }
    pub fn dimensions(&self) -> usize { self.bundle.manifest.dim as usize }
    ...
    pub fn search_vector(&self, query: &[f32], k: usize) -> Vec<(u32, f32)> {
        if k == 0 || query.len() != self.dimensions() { return Vec::new(); }
        ...
        if dtype == "f16" {
            ...
            res.sort_by(|a,b| b.1.partial_cmp(&a.1).unwrap_or(Ordering::Equal).then_with(|| a.0.cmp(&b.0)));
            res
        }
    }
    ...
    fn search_vector_f16(&self, query: &[f32], k: usize) -> Vec<(u32,f32)> {
        ...
        #[derive(Default)]
        struct Tk { k: usize, heap: std::collections::BinaryHeap<std::cmp::Reverse<(ordered_float::OrderedFloat<f32>,u32)>> }
        impl Tk {
            ...
            fn push(&mut self, s: f32, id: u32) {
                ...
                if self.heap.len() < self.k { self.heap.push(item); }
                else if let Some(mut top) = self.heap.peek_mut() { if (item.0).0 > (top.0).0 { *top = item; } }
            }
            fn merge(mut self, other: Self) -> Self { for it in other.heap.into_iter() { self.push(((it.0).0).0, (it.0).1); } self }
        }
        let (heap, _) = (0..n as u32).into_par_iter()
            .with_min_len(1024)
            .fold(|| (Tk::new(k), vec![0f32; dim]), |(mut tk, mut buf), id| {
    ...
    pub fn search_hybrid(&self, query_vec: &[f32], query_text: &str, k: usize, mut vector_weight: f32) -> Vec<(u32, f32)> {
        if k == 0 || query_vec.len() != self.dimensions() { return Vec::new(); }
