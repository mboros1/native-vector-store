...
fn golden_bundle_fields_no_embeddings_in_meta() {
    ...
    let manifest: nvs_core::manifest::Manifest = {
        ...
        File::open(&manifest_path).unwrap().read_to_string(&mut s).unwrap();
    ...
    for _ in 0..block_count {
        ...
        p += 16;
        assert!(doc_count >= 1);
        if codec == 0 { assert!(decomp_size as usize <= inferred_block_size(&mbuf, block_count)); }
        headers.push((comp_size as usize, decomp_size as usize, doc_count as usize, codec));
    }
    ...
    // parse first record
    let mut q = 0usize;
    let id_len = u32::from_le_bytes(block[q..q + 4].try_into().unwrap()) as usize; q += 4;
    let _id = String::from_utf8(block[q..q + id_len].to_vec()).unwrap(); q += id_len;
    let text_len = u32::from_le_bytes(block[q..q + 4].try_into().unwrap()) as usize; q += 4;
    let _text = String::from_utf8(block[q..q + text_len].to_vec()).unwrap(); q += text_len;
    let meta_len = u32::from_le_bytes(block[q..q + 4].try_into().unwrap()) as usize; q += 4;
    let meta_json = String::from_utf8(block[q..q + meta_len].to_vec()).unwrap();
    assert!(!meta_json.contains("\"embedding\""), "embedding should not be in meta by default");
