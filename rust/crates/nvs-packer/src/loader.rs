...
pub fn extract_embedding_and_meta(meta: JsonValue) -> Option<(Vec<f32>, Option<JsonMap<String, JsonValue>>)> {
    ...
    if let Some(mut m) = meta.as_object().cloned() {
        if let Some(emb) = m.remove("embedding") {
            if let Some(arr) = emb.as_array() {
                ...
                for x in arr {
                    if let Some(f) = x.as_f64() { v.push(f as f32); } else if let Some(i) = x.as_i64() { v.push(i as f32); } else { return None; }
                }
...
pub fn build_doc_from_raw(r: InputDocRaw, idx_salt: Option<usize>) -> Option<Doc> {
    ...
    let id = r.id.unwrap_or_else(|| {
        ...
        if let Some(s) = idx_salt { h ^= s as u64; }
        format!("doc-{h:016x}")
    });
    Some(Doc { id, text, embedding, meta: meta_other })
}
            ...
            if s.trim_start().starts_with('[') {
                ...
                for (i, v) in arr.into_iter().enumerate() {
                    ...
                    match serde_json::from_value::<InputDocRaw>(v) {
                        Ok(r) => {
                            if let Some(doc) = build_doc_from_raw(r, Some(i)) { docs.push(doc); }
                            else { skipped += 1; eprintln!("{} skipping invalid/empty doc ({}:#{})", console::style("! ").yellow(), path.display(), i); }
                        }
                ...
                match serde_json::from_str::<InputDocRaw>(&s) {
                    Ok(r) => {
                        ...
                        if let Some(doc) = build_doc_from_raw(r, None) { docs.push(doc); receipts.push((fname, 1)); }
                        else { skipped += 1; eprintln!("{} skipping doc without embedding/metadata ({})", console::style("! ").yellow(), path.display()); receipts.push((fname, 0)); }
                    }
                    Err(e) => { skipped += 1; eprintln!("{} skipping invalid doc ({}) — {}", console::style("! ").yellow(), path.display(), e); receipts.push((path.display().to_string(), 0)); }
                }
            }
        }
    }
...
pub fn read_docs_fast(input_dir: &Path, mmap_threshold: usize) -> Result<(Vec<Doc>, Vec<(String, usize)>)> {
    use walkdir::WalkDir;
            ...
            for entry in WalkDir::new(&input_dir).into_iter().filter_map(|e| e.ok()) {
                ...
                let job = if md.len() as usize <= mmap_threshold {
                    match File::open(&path).and_then(|f| unsafe { Mmap::map(&f) }.map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))) {
                        ...
                        Err(_) => match std::fs::read(&path) { Ok(v) => Job { _path: path, buf: Buf::Vec(v) }, Err(_) => continue }
                    }
                } else {
    ...
    for _ in 0..nthreads {
        ...
        handles.push(thread::spawn(move || -> (Vec<Doc>, usize, Vec<(String, usize)>) {
            ...
            while let Ok(job) = rx.recv() {
                    ...
                    where M: de::MapAccess<'de> {
                        ...
                        if let Some((embedding, meta_other)) = metadata.and_then(super::loader::extract_embedding_and_meta) {
                            ...
                            if !embedding.is_empty() {
                                ...
                                self.out.push(Doc { id: the_id, text: the_text, embedding, meta: meta_other });
                            } else { *self.skipped += 1; }
                        } else { *self.skipped += 1; }
                        Ok(())
                    }
                ...
                let res = de.deserialize_any(StreamVisitor { out: &mut out, skipped: &mut skipped });
                if res.is_err() {
                    ...
                    match serde_json::from_slice::<InputDocRaw>(bytes) {
                        ...
                        Err(_) => { receipts.push((file_name, 0)); }
                    }
                } else {
    ...
    for h in handles { let (mut d, s, mut r) = h.join().unwrap(); docs.append(&mut d); skipped_total += s; receipts_all.append(&mut r); }
    pb.finish_with_message(format!("Loaded {} docs (skipped {})", docs.len(), skipped_total));
