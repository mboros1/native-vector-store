...
mod tests {
    ...
    fn write_vectors_f32_and_f16() {
        ...
        let docs = vec![
            Doc { id: "a".into(), text: "t".into(), embedding: vec![1.0, 2.0, 3.0], meta: None },
            Doc { id: "b".into(), text: "t".into(), embedding: vec![4.0, 5.0, 6.0], meta: None },
        ];
        let dim = 3;
        // f32
    ...
    fn write_meta_manifest_checksums_receipts() {
        ...
        let docs = vec![
            Doc { id: "d1".into(), text: "hello".into(), embedding: vec![0.0, 1.0], meta: Some(serde_json::Map::new()) },
            Doc { id: "d2".into(), text: "world".into(), embedding: vec![1.0, 0.0], meta: None },
        ];
        ...
        let mut s = String::new();
        File::open(dir.join("manifest.json")).unwrap().read_to_string(&mut s).unwrap();
        ...
        File::open(dir.join("receipts.txt")).unwrap().read_to_string(&mut s2).unwrap();
