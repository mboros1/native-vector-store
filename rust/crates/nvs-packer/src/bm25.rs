...
pub fn build_local_maps(
    ...
    local_maps.par_iter_mut().enumerate().for_each(|(ci, local)| {
        let start = ci * chunk_size; if start >= n { return; }
        let end = std::cmp::min(n, start + chunk_size);
        for i in start..end { for (term, count) in doc_tfs[i].iter() { local.entry(term.clone()).or_default().push((i, *count)); } }
    });
    let t_local = t_local_start.elapsed();
    (local_maps, t_local, threads)
}

pub struct BucketOut { pub terms: Vec<String>, pub postings: Vec<u8>, pub lex: Vec<(u64, u32, u32)> }
...
pub fn bucket_kway_merge(
    ...
    (0..buckets).into_par_iter().for_each(|b| {
        ...
        for term in &terms_b {
            ...
            for loc in local_maps { if let Some(vec) = loc.get(term) { slices.push(vec); pos.push(0); } }
            let mut prev = 0usize; let mut len: u32 = 0; let start = postings_b.len() as u64;
            loop {
                let mut best = usize::MAX; let mut which = usize::MAX;
                for i in 0..slices.len() { if pos[i] < slices[i].len() { let d = slices[i][pos[i]].0; if d < best { best = d; which = i; } } }
    ...
    for b in 0..buckets { if let Some(v) = bucket_out[b].lock().unwrap().take() { out.push(v); } else { out.push(BucketOut { terms: Vec::new(), postings: Vec::new(), lex: Vec::new() }); } }
    (out, t_merge)
}
...
pub fn assemble_and_write(
    ...
    for b in &buckets_vec { total_terms += b.terms.len(); if let Some((off, len, _)) = b.lex.last().copied() { total_post_bytes += (off as usize) + (len as usize) * 8; } }
    ...
    let mut global_off: u64 = 0; let mut run_bucket: Option<usize> = None; let mut run_start = 0usize; let mut run_bytes = 0usize; let mut run_expected_next_off = 0usize;
    loop {
        let mut best_b = usize::MAX; let mut best_term: Option<&str> = None;
        for b in 0..buckets {
            let h = heads[b]; let outb = &buckets_vec[b];
            if h < outb.terms.len() { let t = &outb.terms[h]; if best_term.map_or(true, |cur| t.as_str() < cur) { best_term = Some(t.as_str()); best_b = b; } }
        }
        ...
        if run_bucket == Some(best_b) && start == run_expected_next_off { run_bytes += bytes; run_expected_next_off += bytes; }
        else { if let Some(rb) = run_bucket { let src = &buckets_vec[rb].postings[run_start..run_start + run_bytes]; postings.extend_from_slice(src); } run_bucket = Some(best_b); run_start = start; run_bytes = bytes; run_expected_next_off = start + bytes; }
    ...
    { let mut pf = std::fs::File::create(out.join("postings.bin"))?; pf.write_all(&postings)?; let mut lf = std::fs::File::create(out.join("lexicon.bin"))?; lf.write_all(&lexicon)?; }
    let t_io = t_io_start.elapsed();
    let postings_entries_count: usize = postings.len() / 8; let terms: Vec<String> = buckets_vec.into_iter().flat_map(|b| b.terms).collect();
...
pub fn tokenize_bm25_into(text: &str, tf: &mut FxHashMap<String, u32>) -> usize {
    ...
    let mut buf = String::with_capacity(32); let mut kept = 0usize;
    let mut flush = |buf: &mut String| {
        ...
        if let Some(norm) = bm25_normalize_token(&buf) {
            if !nvs_core::tokenizer::is_stopword(&norm) { *tf.entry(norm).or_insert(0) += 1; kept += 1; }
        }
        buf.clear();
    };
    ...
    while let Some(ch) = chars.next() {
        match ch {
            ...
            '-' => {
                let mut it = chars.clone(); let mut consumed = 0; let mut is_break = false;
                while let Some(nc) = it.next() { if nc == '\n' { is_break = true; consumed += 1; break; } else if nc == '\r' || nc == '\t' || nc == ' ' { consumed += 1; continue; } else { break; } }
                if is_break { for _ in 0..consumed { let _ = chars.next(); } flush(&mut buf); } else { buf.push('-'); }
            }
...
pub fn write_bm25_and_terms(docs: &[Doc], out: &Path, bm25_buckets: usize) -> Result<(f64, Vec<String>, usize, usize, Bm25Stats)> {
    ...
    Ok((avgdl, terms, postings_entries_count, total_tokens, Bm25Stats { tf: t_tf, local: t_local, merge: t_merge, write: t_assemble + t_io }))
}
...
mod tests {
    ...
    fn write_bm25_outputs() {
        ...
        let docs = vec![
            Doc { id: "d1".into(), text: "alpha beta beta".into(), embedding: vec![0.0], meta: None },
            Doc { id: "d2".into(), text: "beta gamma".into(), embedding: vec![0.0], meta: None },
        ];
        let (avgdl, terms, postings, total_tokens, _stats) = write_bm25_and_terms(&docs, &dir, 4).unwrap();
        assert!(avgdl > 0.0);
        assert!(terms.len() > 0);
        assert!(postings > 0);
