...
use crate::objects::{
    as_dict, as_name, is_alpha, parse_name, parse_number, parse_string, resolve, skip_ws, PdfDoc,
    PdfValue,
};
            ...
            Tok::Op(op) => match op.as_str() {
                ...
                "Tm" => {
                    if last_nums.len() >= 6 {
                        ...
                        if let Some(prev) = ts.prev_y {
                            let dy = f - prev;
                            let thresh = if ts.leading.abs() > 0.0 {
                                0.8 * ts.leading.abs()
                            } else {
                                0.5 * ts.font_size.max(1.0)
                            };
                            if dy.abs() >= thresh {
                                out.push('\n');
                            }
                ...
                "Td" | "TD" => {
                    if last_nums.len() >= 2 {
                        ...
                        ts.tm_e += dx;
                        ts.tm_f += dy;
                        let thresh_nl = if ts.leading.abs() > 0.0 {
                            0.8 * ts.leading.abs()
                        } else {
                            0.5 * ts.font_size.max(1.0)
                        };
                ...
                // Text leading
                "TL" => {
                    if let Some(tl) = last_nums.last().copied() {
                        ts.leading = tl;
                    }
                    last_nums.clear();
                }
                "Tw" => {
                    if let Some(w) = last_nums.last().copied() {
                        ts.word_spacing = w;
                    }
                    last_nums.clear();
                }
                "Tc" => {
                    if let Some(c) = last_nums.last().copied() {
                        ts.char_spacing = c;
                    }
                    last_nums.clear();
                }
                "Tz" => {
                    if let Some(z) = last_nums.last().copied() {
                        ts.h_scale = (z / 100.0).max(0.01);
                    }
                    last_nums.clear();
                }
                "Tj" => {
                        ...
                        if let Some(xv) = xobjects.get(&nm) {
                            ...
                            if let PdfValue::Stream { dict, data } = rv {
                                if dict.get("Subtype").and_then(|v| as_name(v)) == Some("Form") {
                                    ...
                                    if let Some(res) =
                                        dict.get("Resources").and_then(|v| as_dict(v))
                                    {
                                        if let Some(xd) =
                                            res.get("XObject").and_then(|v| as_dict(v))
                                        {
                                            for (k, v) in xd {
                                                let rv = resolve(doc, v, 0)
                                                    .unwrap_or_else(|_| v.clone());
                                                sub_xobjs.insert(k.clone(), rv);
                                            }
                                        }
                                        if let Some(fdict) =
                                            res.get("Font").and_then(|v| as_dict(v))
                                        {
                                            for (name, fv) in fdict {
                                                let rf = resolve(doc, fv, 0)
                                                    .unwrap_or_else(|_| fv.clone());
                                                if let Some(fd) = as_dict(&rf) {
                                                    ...
                                                    if let Some(tu) = fd.get("ToUnicode") {
                                                        let rf2 = resolve(doc, tu, 0)
                                                            .unwrap_or_else(|_| tu.clone());
                                                        if let PdfValue::Stream {
                                                            dict: sdict,
                                                            data,
                                                        } = rf2
                                                        {
                                ...
                                crate::stats::add_fonts_duration(tf.elapsed().as_nanos() as u128);
                                    ...
                                    let ti = Instant::now();
                                    let sub = interpret_text_with_resources_depth(
                                        doc,
                                        &sub_xobjs,
                                        &sub_fonts,
                                        &dec,
                    ...
                    if next == "Tj" {
                        if let Some(ref fname) = current_font {
                            if let Some(fi) = fonts.get(fname) {
                                ...
                                append_bytes_as_text(&mut out, &s);
                            }
                        } else {
                            append_bytes_as_text(&mut out, &s);
                        }
                        ...
                        // apostrophe operator: newline + show
                        if !out.ends_with('\n') {
                            out.push('\n');
                        }
                        if let Some(ref fname) = current_font {
                        ...
                        // double-quote operator: set Tw/Tc from last_nums, newline, show
                        if last_nums.len() >= 2 {
                        ...
                        Tok::Str(s) => {
                            if let Some(ref fname) = current_font {
                                if let Some(fi) = fonts.get(fname) {
                                    ...
                                    append_bytes_as_text(&mut arr_text, &s);
                                }
                            } else {
                                append_bytes_as_text(&mut arr_text, &s);
                            }
...
fn interpret_text_with_resources(
    doc: &PdfDoc,
    xobjects: &BTreeMap<String, PdfValue>,
    fonts: &BTreeMap<String, FontInfo>,
    content: &[u8],
...
impl<'a> Tokenizer<'a> {
        ...
        match c {
            ...
            b'+' | b'-' | b'.' | b'0'..=b'9' => {
                match parse_number(self.b, self.i) {
                    Ok((n, j, _)) => {
                        self.i = j;
                        Ok(Some(Tok::Num(n as f64)))
                    }
                    Err(_) => {
                        // recover from malformed numeric (e.g., lone '-' before operator); skip one byte and continue
                        self.i += 1;
                        self.next()
                    }
                }
    ...
    fn consume_op(&mut self) {
        if let Some(op) = self.peeked_op.take() {
            let _ = op;
            self.skip_ws();
            let _ = self.next();
        }
