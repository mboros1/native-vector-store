...
use crate::objects::{PdfDoc, PdfValue, as_dict, as_name, resolve};
...
pub fn extract_page_text_with_debug(doc: &PdfDoc, page: (u32,u16)) -> (Option<String>, PageTextDebug) {
    ...
    if let Some(res) = dict.get("Resources").and_then(|v| as_dict(v)) {
        if let Some(fdict) = res.get("Font").and_then(|v| as_dict(v)) {
            ...
            for (_name, fv) in fdict {
                ...
                if let Some(fd) = as_dict(&rf) { if fd.get("ToUnicode").is_some() { with_tu += 1; } }
            }
    ...
    let decode_one = |vv: PdfValue, filters: &mut Vec<String>, preds: &mut Vec<i64>, buffers: &mut Vec<u8>| -> Result<()> {
        if let PdfValue::Stream{ dict: sdict, data } = vv {
            ...
            if let Some(dp) = sdict.get("DecodeParms") { if let Some(p) = get_predictor(dp) { preds.push(p); } }
    ...
    match contents {
        PdfValue::Stream { dict, data } => { let vv = PdfValue::Stream{ dict: dict.clone(), data: data.clone() }; if let Err(e)=decode_one(vv, &mut filters, &mut preds, &mut buffers) { dbg.notes.push(format!("decode: {}", e)); decode_ok=false; } }
        PdfValue::Array(arr) => { for v in arr { let vv = match resolve(doc, v, 0) { Ok(x)=>x, Err(e)=>{ dbg.notes.push(format!("resolve stream: {}", e)); continue } }; if let Err(e)=decode_one(vv, &mut filters, &mut preds, &mut buffers) { dbg.notes.push(format!("decode arr: {}", e)); decode_ok=false; } } }
        PdfValue::Ref(obj, gen) => { let vv = match doc.get_object(*obj, *gen) { Ok(x)=>x, Err(e)=>{ dbg.notes.push(format!("get stream: {}", e)); return (None, dbg) } }; if let Err(e)=decode_one(vv, &mut filters, &mut preds, &mut buffers) { dbg.notes.push(format!("decode ref: {}", e)); decode_ok=false; } }
        _ => {}
    }
...
fn get_predictor(dp: &PdfValue) -> Option<i64> {
    match dp {
        ...
        PdfValue::Array(arr) => { for v in arr { if let Some(i)=get_predictor(v) { return Some(i); } } None }
        _=>None,
    }
}
