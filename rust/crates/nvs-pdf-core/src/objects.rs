...
impl PdfDoc {
    ...
    pub(crate) fn expand_object_streams(&mut self) -> Result<()> {
        ...
        for (obj, gen) in keys {
            if let Some(range) = self.objects.get(&(obj, gen)).cloned() {
                if let Ok(PdfValue::Stream { dict, data }) =
                    self.parse_object_at_range(range.clone())
                {
                    if dict.get("Type").and_then(|v| as_name(v)) == Some("ObjStm") {
                        // Try primary decode
                        let decoded = match crate::streams::get_stream_data_with_filters(
                            &dict,
                            data.clone(),
                        ) {
                            Ok(v) => v,
                            Err(_e1) => {
                                ...
                                }) = self.parse_stream_at_range_scan(range.clone())
                                {
                                            ...
                                            if let Some(PdfValue::Int(len)) = dict.get("Length") {
                                                if let Some(v3) = self.bruteforce_flate_slice(
                                                    ...
                                                    *len as usize,
                                                    ...
                                                    return Err(anyhow!(
                                                        "ObjStm decode failed after brute-force"
                                                    ));
                                                }
                                            } else {
                                    ...
                                    return Err(anyhow!(
                                        "ObjStm decode failed and scan fallback unavailable"
                                    ));
                                }
                            }
                        };
                        let n = dict
                            ...
                            .ok_or_else(|| anyhow!("ObjStm missing N"))?;
                        let first = dict
                            .get("First")
                            .and_then(|v| match v {
                                PdfValue::Int(i) => Some(*i as usize),
                            ...
                            .ok_or_else(|| anyhow!("ObjStm missing First"))?;
                        ...
                        for k in 0..n {
                            ...
                            let off = nums[2 * k + 1];
                            let off_next = if k + 1 < n {
                                nums[2 * (k + 1) + 1]
                            } else {
                                body.len()
                            };
                            if off >= body.len() || off_next > body.len() || off >= off_next {
                                continue;
                            }
            ...
            if let Some(range) = self.objects.get(&(obj, gen)).cloned() {
                    ...
                    self.parse_object_at_range(range.clone())
                {
                    if dict.get("Type").and_then(|v| as_name(v)) == Some("XRef") {
                        if let Ok(decoded) =
                            crate::streams::get_stream_data_with_filters(&dict, data)
                        {
    ...
    fn bruteforce_flate_slice(
        ...
        i = skip_ws(bytes, i);
        let (_val, j) = parse_value(bytes, i).ok()?;
        let j = skip_ws(bytes, j);
        if bytes.get(j..j + 6) != Some(b"stream") {
            return None;
        }
        ...
        for d in deltas {
            ...
            if end <= bytes.len() && end > k + 8 {
                ...
                if let Ok(v) = crate::filters::decode_flate_tolerant(slice) {
                    return Some(v);
                }
            }
        }
    ...
    fn parse_xref_stream_entries(&mut self, dict: &BTreeMap<String, PdfValue>, data: &[u8]) {
        ...
        let w = match dict.get("W").and_then(|v| as_array(v)) {
            Some(arr) if arr.len() >= 3 => arr
                ...
                .take(3)
                .map(|v| match v {
                    PdfValue::Int(i) => *i as usize,
                    PdfValue::Real(f) => (*f as i64) as usize,
                    _ => 0,
                })
                .collect::<Vec<_>>(),
            _ => return,
        };
        let w0 = *w.get(0).unwrap_or(&0);
        let w1 = *w.get(1).unwrap_or(&0);
        let w2 = *w.get(2).unwrap_or(&0);
        ...
        if let Some(idx) = dict.get("Index").and_then(|v| as_array(v)) {
            ...
            while let (Some(a), Some(b)) = (it.next(), it.next()) {
                ...
                if count > 0 {
                    indexes.push((start, count));
                }
            }
        } else if let Some(PdfValue::Int(sz)) = dict.get("Size") {
            indexes.push((0, *sz as usize));
        }
        if indexes.is_empty() {
            return;
        }
        ...
        for (start, count) in indexes {
            for i in 0..count {
                if off + w0 + w1 + w2 > data.len() {
                    return;
                }
                let t = if w0 == 0 {
                    1u64
                } else {
                    read_uint_be(&data[off..off + w0])
                };
                off += w0;
                let f1 = if w1 == 0 {
                    0u64
                } else {
                    read_uint_be(&data[off..off + w1])
                };
                off += w1;
                let f2 = if w2 == 0 {
                    0u64
                } else {
                    read_uint_be(&data[off..off + w2])
                };
                off += w2;
                ...
                match t {
                    0 => { /* free */ }
                    1 => {
                        // uncompressed object at offset f1, gen f2
                        let offset = f1 as usize;
                        let gen = f2 as u16;
                        self.add_object_range_if_missing(objnum, gen, offset);
                    }
                    2 => {
                        // compressed object in object stream f1 at index f2
                        // Ensure the object stream itself is present; will be expanded later
                        let objstm = f1 as u32;
                        self.add_object_range_if_any(objstm);
                    }
                    _ => {}
                }
            }
    ...
    fn add_object_range_if_any(&mut self, obj: u32) {
        ...
        if self.objects.keys().any(|(o, _g)| *o == obj) {
            return;
        }
        ...
        if let Some(pos) = find_token(&self.data, 0, needle.as_bytes()) {
            if let Some(end) = find_token(&self.data, pos, b"endobj") {
                self.objects.insert((obj, 0u16), pos..end + 6);
            }
        }
    }

    fn add_object_range_if_missing(&mut self, obj: u32, gen: u16, offset: usize) {
        if self.objects.contains_key(&(obj, gen)) {
            return;
        }
...
impl PdfDoc {
    fn populate_from_startxref(&mut self) {
        ...
        while i > 0 {
            if self.data[i..].starts_with(b"startxref") {
                pos = Some(i);
                break;
            }
            i -= 1;
        ...
        while j < self.data.len() && is_ws(self.data[j]) {
            j += 1;
        }
        let num_start = j;
        ...
        let Ok(off) = off_str.parse::<usize>() else {
            return;
        };
                ...
                if let PdfValue::Stream { dict, data } = val {
                    if dict.get("Type").and_then(|v| as_name(v)) == Some("XRef") {
                            ...
                            crate::streams::get_stream_data_with_filters(&dict, data)
                        {
                            ...
                            if let Some(PdfValue::Int(prev)) = dict.get("Prev") {
                                queue.push(*prev as usize);
                            }
                            if let Some(PdfValue::Int(xrs)) = dict.get("XRefStm") {
                                queue.push(*xrs as usize);
                            }
                        }
                    }
                }
...
pub(crate) fn parse_dict(
    bytes: &[u8],
    mut i: usize,
) -> Result<(BTreeMap<String, PdfValue>, usize)> {
    ...
    loop {
        ...
        if bytes.get(i) != Some(&b'/') {
            return Err(anyhow!("dict key expected"));
        }
...
pub(crate) fn parse_number_or_ref(bytes: &[u8], i: usize) -> Result<(PdfValue, usize)> {
    ...
    if is_real1 {
        Ok((PdfValue::Real(n1 as f64), j1))
    } else {
        Ok((PdfValue::Int(n1), j1))
    }
}
...
pub(crate) fn starts_with(bytes: &[u8], i: usize, token: &[u8]) -> bool {
    bytes.get(i..i + token.len()) == Some(token)
}
pub(crate) fn find_token(bytes: &[u8], mut i: usize, token: &[u8]) -> Option<usize> {
...
pub(crate) fn parse_uint(bytes: &[u8], mut i: usize) -> (Option<i64>, usize) {
    let start = i;
...
pub(crate) fn is_alpha(b: u8) -> bool {
    (b'a'..=b'z').contains(&b) || (b'A'..=b'Z').contains(&b) || b'*' == b
}

// Utility helpers
pub(crate) fn as_name(v: &PdfValue) -> Option<&str> {
...
pub(crate) fn as_dict(v: &PdfValue) -> Option<&BTreeMap<String, PdfValue>> {
    if let PdfValue::Dict(ref d) = v {
        Some(d)
    } else {
        None
    }
}
pub(crate) fn as_array(v: &PdfValue) -> Option<&Vec<PdfValue>> {
    if let PdfValue::Array(ref a) = v {
        Some(a)
    } else {
        None
    }
}
pub(crate) fn resolve<'a>(doc: &'a PdfDoc, v: &'a PdfValue, depth: usize) -> Result<PdfValue> {
    if depth > 8 {
        return Err(anyhow!("ref recursion"));
    }
    if let PdfValue::Ref(o, g) = v {
        return doc.get_object(*o, *g);
    }
    Ok(v.clone())
}
