use crate::objects::{as_array, as_dict, as_name, parse_indirect_object, PdfDoc, PdfValue};
...
pub fn collect_page_object_ids(doc: &PdfDoc) -> Vec<(u32, u16)> {
    ...
    for ((obj, gen), val) in iter_all_objects_values(doc) {
        if let Some(d) = as_dict(&val) {
            if d.get("Type").and_then(|v| as_name(v)) == Some("Page") {
                pages.push((obj, gen));
            }
        }
    }
...
pub fn collect_pages_via_tree(doc: &PdfDoc) -> Result<Vec<(u32, u16)>> {
    ...
    let pages_val = cat_dict
        .get("Pages")
        .ok_or_else(|| anyhow!("Catalog missing Pages"))?;
    let mut out = Vec::new();
...
fn iter_all_objects_values(doc: &PdfDoc) -> Vec<((u32, u16), PdfValue)> {
    ...
    for (&(obj, gen), range) in doc.objects.iter() {
        if let Ok(val) = parse_indirect_object(&doc.data[range.clone()]) {
            out.push(((obj, gen), val));
        }
    }
    for (&(obj, gen), buf) in doc.inline_objects.iter() {
        if let Ok(val) = parse_indirect_object(buf) {
            out.push(((obj, gen), val));
        }
    }
    out
...
fn traverse_pages_node(
    ...
    if depth > 64 {
        return Err(anyhow!("pages tree too deep"));
    }
    let val = match node {
        PdfValue::Ref(o, g) => doc.get_object(*o, *g)?,
        _ => node.clone(),
    };
    ...
    match d.get("Type").and_then(|v| as_name(v)) {
        Some("Pages") => {
            if let Some(kids) = d.get("Kids").and_then(|v| as_array(v)) {
                for kid in kids {
                    traverse_pages_node(doc, kid, out, depth + 1)?;
                }
            }
            Ok(())
        }
        Some("Page") => {
            if let PdfValue::Ref(o, g) = node {
                out.push((*o, *g));
            }
            Ok(())
        }
