...
impl<'a> Tokenizer<'a> {
    pub fn new(b: &'a [u8]) -> Self { Self { b, i: 0, peeked_op: None } }
    ...
    pub fn next(&mut self) -> Result<Option<Tok>> {
        ...
        let c = self.b[self.i];
        Ok(Some(match c {
            b'(' => { let (s, j) = parse_string(self.b, self.i + 1)?; self.i = j; Tok::Str(s) }
            b'[' => { self.i += 1; Tok::ArrStart }
            b']' => { self.i += 1; Tok::ArrEnd }
            b'/' => { let (n, j) = parse_name(self.b, self.i + 1)?; self.i = j; Tok::Name(n) }
            b'\'' => { self.i += 1; Tok::Op("'".to_string()) }
            b'"' => { self.i += 1; Tok::Op("\"".to_string()) }
            b'+' | b'-' | b'.' | b'0'..=b'9' => {
                match parse_number(self.b, self.i) {
                    Ok((n, j, _)) => { self.i = j; Tok::Num(n as f64) }
                    Err(_) => { self.i += 1; return self.next(); }
                }
            }
            _ => {
                ...
                if j > start {
                    ...
                    self.i = j;
                    Tok::Op(op)
                } else { self.i += 1; return self.next(); }
            }
        }))
    }
    ...
    pub fn skip_inline_image_after_id(&mut self) {
        ...
        while self.i + 1 < self.b.len() {
            ...
            if self.b[self.i] == b'E' && self.b[self.i + 1] == b'I' {
                ...
                if crate::objects::is_ws(prev) && (crate::objects::is_ws(next) || !is_alpha(next)) { self.i += 2; break; }
            }
            self.i += 1;
        }
    }
