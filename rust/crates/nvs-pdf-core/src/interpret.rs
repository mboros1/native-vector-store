...
use crate::fonts::{parse_tounicode_cmap, map_bytes_with_tounicode_or_base};
use crate::resources::{PageResources, FontInfo};

pub fn append_bytes_as_text(out: &mut String, s: &[u8]) { match String::from_utf8(s.to_vec()) { Ok(t) => out.push_str(&t), Err(_) => { for &b in s { out.push(b as char); } } } }
...
pub fn interpret_text_with_resources(doc: &PdfDoc, xobjects: &BTreeMap<String, PdfValue>, fonts: &BTreeMap<String, FontInfo>, content: &[u8]) -> Result<String> {
        ...
        match tok {
            Tok::Op(op) => match op.as_str() {
                ...
                "Td" | "TD" => {
                    if last_nums.len() >= 2 {
                        ...
                        if dy.abs() > 0.0 { if let Some(prev) = ts.prev_y { let thresh = if ts.leading.abs() > 0.0 { 0.8 * ts.leading.abs() } else { 0.5 * ts.font_size.max(1.0) }; if dy.abs() >= thresh { out.push('\n'); } } ts.prev_y = Some(ts.tm_f + dy); }
                        if dx > 10.0 { out.push(' '); }
                    }
                    last_nums.clear();
                }
            ...
            Tok::Str(s) => {
                if let Some(next) = toks.peek_op()? {
                    ...
                    } else if next == "\"" { // double-quote operator
                        if last_nums.len() >= 2 {
