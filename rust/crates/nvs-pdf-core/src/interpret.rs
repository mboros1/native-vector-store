use crate::fonts::{map_bytes_with_tounicode_or_base, parse_tounicode_cmap};
use crate::objects::{
    as_dict, as_name, is_alpha, parse_name, parse_number, parse_string, resolve, skip_ws, PdfDoc,
    PdfValue,
};
use crate::resources::{FontInfo, PageResources};
...
pub fn interpret_contents(doc: &PdfDoc, res: &PageResources, content: &[u8]) -> Result<String> {
    interpret_text_with_resources(doc, &res.xobjects, &res.fonts, content)
}
