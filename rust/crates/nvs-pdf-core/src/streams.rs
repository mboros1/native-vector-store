        ...
        match filter {
            ...
            PdfValue::Array(arr) => {
                ...
                for (idx, f) in arr.iter().enumerate() {
                    ...
                    } else { return Err(anyhow!("filter name expected")); }
                }
                out = cur;
            }
            _ => {}
...
fn apply_filter_with_params(name: &str, data: Vec<u8>, decode_parms: Option<&PdfValue>, index: usize) -> Result<Vec<u8>> {
    ...
    let mut decoded = match name {
        ...
        "LZWDecode" => {
            ...
            if let Some(params) = decode_parms { if let Some(dp) = get_decode_params_for_index(params, index) { if let Some(ec) = dp.get("EarlyChange").and_then(as_int) { early_change = ec != 0; } } }
...
fn as_int(v: &PdfValue) -> Option<i64> { match v { PdfValue::Int(i) => Some(*i), PdfValue::Real(f) => Some(*f as i64), _ => None } }

fn get_decode_params_for_index(decode_parms: &PdfValue, index: usize) -> Option<&BTreeMap<String, PdfValue>> {
    match decode_parms {
        PdfValue::Dict(ref d) => Some(d),
        PdfValue::Array(ref arr) => {
            if index < arr.len() {
                if let PdfValue::Dict(ref d) = arr[index] { return Some(d); }
            }
            None
        }
        _ => None,
...
fn apply_predictor(data: &[u8], predictor: u32, colors: u32, bpc: u32, columns: u32) -> Result<Vec<u8>> {
    if predictor == 2 { // TIFF predictor (horizontal differencing)
        let bpp = ((colors * bpc + 7) / 8) as usize;
        if bpp == 0 { return Ok(data.to_vec()); }
    ...
    if predictor >= 10 {
        ...
        if row_bytes == 0 { return Ok(data.to_vec()); }
        let mut out = Vec::with_capacity(data.len());
        let mut i = 0usize;
        let bpp = ((colors * bpc + 7) / 8) as usize;
        while i < data.len() {
            if i >= data.len() { break; }
            let filter = data[i]; i += 1;
            if i + row_bytes > data.len() { break; }
            let row = &data[i..i+row_bytes];
            let mut dst = vec![0u8; row_bytes];
            match filter {
                0 => { dst.copy_from_slice(row); }
                1 => { // Sub
                ...
                2 => { // Up
                    ...
                    let prev_offset = if out.len() >= row_bytes { out.len() - row_bytes } else { usize::MAX };
                    for x in 0..row_bytes {
                        let up = if prev_offset != usize::MAX { out[prev_offset + x] } else { 0 };
                ...
                4 => { // Paeth
                    ...
                    for x in 0..row_bytes {
                        ...
                        let b = if prev_offset != usize::MAX { out[prev_offset + x] } else { 0 };
                        let c = if prev_offset != usize::MAX && x >= bpp { out[prev_offset + x - bpp] } else { 0 };
                        dst[x] = row[x].wrapping_add(paeth(a, b, c));
                    }
                }
                _ => { dst.copy_from_slice(row); }
            }
...
fn paeth(a: u8, b: u8, c: u8) -> u8 {
    let a = a as i32; let b = b as i32; let c = c as i32;
    let p = a + b - c;
    let pa = (p - a).abs();
    let pb = (p - b).abs();
    let pc = (p - c).abs();
